...
class PapakiDomainNameSearch{
 ...
 //$exts=array('.'.$this->tld);
	...
	$jsonarray=array("request"=>array("type"=>$type,"apiKey"=>encodetolatin($this->apikey),"username"=>'',"password"=>'',"domain"=>encodetolatin($this->domainName),"lang"=>'el',"test"=>'False',"extensions"=>array("ext"=>$exts)));
	...
	function parseResponse($executed = true){
		...
		//echo($this->responsearray->response);
		if($codeNode != 1000){
		...
		if ($this->type == _TYPE_DS){
			...
			$myarrayNotav= $this->responsearray->response->notAvailableDomains->domain;
			 $json = new Services_JSON();
			// print_r($myarrayNotav);


			if(is_array($myarrayNotav)){
			 	foreach ($myarrayNotav as $i => $value) {
  					array_push($this->arrayNotAvDomains, $value);
				}
			...
			$myarrayav= $this->responsearray->response->availableDomains->domain;
			//print_r($myarrayav);
			if(is_array($myarrayav) ){

				 	 foreach ($myarrayav as $i => $value) {
  					array_push($this->arrayAvDomains, $value);
				}
	...
	function getExtensions(){
		...
		$selectedExt = array();
		 ...
		 for ($i=0;$i<=count($exts)-1;$i++){
			if ($_POST[$this->checkBoxPrefix.$exts[$i]] != ""){

				array_push($selectedExt, $_POST[$this->checkBoxPrefix.$exts[$i]]);
			}
		}
